...
//        updateListAndEmptyView(filteredList);
        ...
        sp_VM = new ViewModelProvider(this).get(SanPhamVM.class);
        sp_VM.getProductListLiveData().observe(this, new Observer<List<SanPham>>() {
            @Override
            public void onChanged(List<SanPham> sanPhams) {
                for (SanPham sp : sanPhams) {
                    if (sp.getNhomSanPham().getId().equals(nhomSPId)) {
                        sanPhamList.add(sp);
                    }
                }
                Log.d("Listsp",String.valueOf(sanPhamList.size()));
//                sanPhamAdapter = new SanPhamAdapter(sanPhamList, DanhMucActivity.this, getLayoutInflater());
//                recyclerView.setAdapter(sanPhamAdapter);
            }
        });
